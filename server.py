...
def send_all_clients(
    msg, sender
):  # send data to ALL clients connected. msg: message to be sent; sender: sender of the relayed message
    for client in clients_connected:
        if (
            client != sender and clients_connected != []
        ):  # not relay message to original sender
            client.send(msg)
...
def handle_client(conn, addr):
    ...
    discon_alert = (
        medium_prior("[CLIENT DISCONNECTED]:") + f" {addr} has disconnected. "
    )
    ...
    while connected:
        ...
        if msg_length:
            ...
            if msg != DISCONNECT:
                print(
                    low_prior("(" + str(addr) + "):") + f" {msg}"
                )  # output CLIENT's message
